...
この章では、まず `Deployment` だけを Manifest で作成します。
次の章では、この章で作った Deployment に対して `Service` を追加します。
...
この時点では、まだ Service を作っていないため、ブラウザや `curl` でアプリへアクセスする入口はありません。
次の章で Service を追加して、外からアクセスできるようにします。
...
ここまでで、`kubectl apply -f my-httpd.yaml` を使って Deployment を作成しました。
...
```bash
kubectl create deployment my-httpd --image=docker.io/library/httpd:2.4
...
すでに Manifest と同じ状態になっているため、余計に Pod が増えるわけではありません。
...
::: tip
この章で作った `my-httpd` Deployment は、次の章で Service から参照します。
